...
paragraph(a,226,"Implementation checks support the arithmetic result",
    "119 CPU feature checks cover filtering, flux signs and guards, support,\nimplicit conservation, and checkpoint continuation. Separate CPU runner\nand export checks cover shapes, native heights, schedules and provenance.")
text!(a,45,92;text="Boundary of the evidence: synthetic arithmetic and CPU checks are not\nevidence of improved LES fidelity. GPU validation and matched LES remain pending.\nReproducer, exact plotted CSV, and source hashes accompany this section.",fontsize=13,color=GRAY,align=(:left,:top))
finish(f,"02_arithmetic")

f,a=page(3,"HOW TO READ THE COMPARISON", "Judge transport, turbulence,\nand the mean state together",
    "One controlled intervention, three complementary tests of physical behavior.\nNo completed SurfaceLayerDiffusivity LES result is being claimed here.")
...
paragraph(a,696,"02  Did the boundary layer improve without losing turbulence?",
    "Place mean wind and temperature beside w², w³ and skewness. Show\nboundary-layer depth and surface flux through time. Retain the fixed 1 m\nGABLS1 median on comparable panels; gray bands show archive spread.")
