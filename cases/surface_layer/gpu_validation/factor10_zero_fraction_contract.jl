...
function zero_fraction_contract(architecture; check)
    ...
    set!(f.momentum_deficit[1], (x,y,z) -> x < 10 ? 0f0 : 0.5f0)
    set!(f.Kᵘ, (x,y,z) -> x < 20 ? 0f0 : 1f0)
    set!(f.momentum_active[1], (x,y,z) -> 10 <= x < 20 ? 0f0 : 1f0)
    set!(f.viscosity_cap_active[1], (x,y,z) -> x >= 30 ? 1f0 : 0f0)
    set!(f.scalar_deficit.ρθ[1], (x,y,z) -> x < 20 ? 0f0 : 0.5f0)
    set!(f.tupled_tracer_diffusivities.ρθ, (x,y,z) -> x < 30 ? 0f0 : 1f0)
    set!(f.scalar_active.ρθ[1], (x,y,z) -> 10 <= x < 30 ? 0f0 : 1f0)
    set!(f.diffusivity_cap_active.ρθ[1], (x,y,z) -> x >= 30 ? 1f0 : 0f0)
