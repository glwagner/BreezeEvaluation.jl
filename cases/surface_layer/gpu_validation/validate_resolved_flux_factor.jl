...
function main()
    ...
    gpu = MODE == "gpu"
    prefix = gpu ? "GPU" : "CPU"
    mode = gpu ? "gpu_resolved_flux_factor" : "cpu_resolved_flux_factor"
    try
        ...
        default = SurfaceLayerDiffusivity(Float32)
        one = SurfaceLayerDiffusivity(Float32; resolved_flux_factor=1)
        two = SurfaceLayerDiffusivity(Float32; resolved_flux_factor=2)
        args = (-0.01f0, 0f0, -0.04f0, 0f0, 12.5f0, 1f0)
        p0, p1, p2 = props(args..., default), props(args..., one), props(args..., two)
        check(p0 == p1, "default differs from explicit one")
        check(p1.deficit == 0.75f0 && p2.deficit == 0.5f0, "factor2 deficit formula")
        check(p1.parallel_resolved_stress == p2.parallel_resolved_stress, "resolved projection was scaled")
        check(scalar(-0.0025f0, -0.01f0, 0.2f0, 12.5f0, 1f0, 1f-8, two).deficit == 0.5f0, "heat factor formula")
        ...
        evidence = Dict("validation_mode" => mode, "case_family" => "GABLS1",
            "all_passed" => true, "factors" => [1.0, 2.0], "changed_path_checks_passed" => true,
