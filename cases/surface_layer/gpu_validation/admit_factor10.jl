...
function admit_factor10(directory, root)
    result = verify_gpu_evidence(directory, root, file_sha256(joinpath(root, "source_sha256.txt")))
    result["validation_mode"] == "gpu_factor10" || error("wrong gate scope")
    ...
    length(evidence["runners"]) == 1 || error("one factor10 full-runner fixture required")
    for (run, factor) in zip(evidence["runners"], (10.0,))
    ...
    println("FACTOR10_GPU_ADMITTED evidence_sha256=", result["evidence_sha256"])
    return result
end
