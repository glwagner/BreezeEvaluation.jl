...
breeze_destination = joinpath(source_root, "Breeze.jl")
...
open(readme, "w") do io
    println(io, "# Surface-layer GPU harness freeze")
    println(io)
    ...
    println(io)
    println(io, "This read-only snapshot is an unsubmitted GPU-validation candidate, not an")
    println(io, "admitted scientific result. The original 15-case GABLS1 registry and earlier")
    println(io, "`surface-layer-freeze-20260920-34e955b` snapshot are not modified.")
end
