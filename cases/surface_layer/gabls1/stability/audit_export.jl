...
# stability-strength provenance line is required for the λ=1 case.
...
for required in ("case_id: $case_id", "nx: 32", "spacing: 12.5", "filter_seconds: 300.0",
                 "wall_filter_seconds: 300.0", "stop_time: 32400.0", "seed: 123",
                 "theta_initial_sha256: 1f5db33f2971ee607ac46b1a014b038a09fc876cc1669394dce023a9aa2f198b")
...
occursin("stability_strength: 1.0", provenance) || error("stability strength mismatch")
...
jldopen(wall_path, "r") do f
    ...
    for name in ("filtered_u", "filtered_v", "filtered_Δθ")
        ...
        for (key, t) in rs
            ...
            size(a)[1:2] == (42, 42) || error("$name wall halo shape")
            all(isfinite, a) || error("$name wall nonfinite at $t")
            push!(means, mean(a[6:37, 6:37, 1]))
