...
ENV["GABLS1_SLD_STABILITY_STRENGTH"] = "2.0"
...
run_directory = joinpath(gate_root, "surface_layer_stab2p0")
setup = build_simulation(; run_directory)
expected_id = "gabls1_n032_weno9_surface_layer_t300_s1_rf1p0_native_stab2p0_wallf300"
stop_seconds = parse(Float64, ENV["GABLS1_SLD_STOP_SECONDS"])

@testset "λ=2 stability-corrected SLD GABLS1 gate" begin
    model = setup.model
    ...
    jldopen(series) do file
        @test file["metadata/surface_layer_stability_strength"] == 2
