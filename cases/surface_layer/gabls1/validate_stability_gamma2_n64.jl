...
length(ARGS) == 2 || error("usage: validate_stability_gamma2_n64.jl none|s1|s2 GATE_ROOT")
kind, gate_root = ARGS
kind in ("none", "s1", "s2") || error("unsupported gate case $kind")
is_sld = kind != "none"
support = kind == "s2" ? 2 : 1
ENV["GABLS1_SLD_ARCH"] = get(ENV, "GABLS1_SLD_ARCH", "gpu")
ENV["GABLS1_SLD_NX"] = "64"
...
expected_id = kind == "none" ? "gabls1_n064_weno9_control_wallf300" :
    "gabls1_n064_weno9_surface_layer_t300_s$(support)_rf1p0_native_stab2p0_wallf300"
stop_seconds = parse(Float64, ENV["GABLS1_SLD_STOP_SECONDS"])

@testset "n64 GABLS1 gamma2 $kind gate" begin
    @test setup.case_id == expected_id
    @test setup.settings.nx == 64
    @test setup.settings.spacing == 6.25
    @test setup.settings.theta_initial_sha256 ==
        "1b6c480e8e682c97ade3db99bcf9559cf1723d500858c600b74c318f4a97c864"
    ...
    if is_sld
        ...
        @info @sprintf("N64_GATE_STABILITY kind=%s t=%.0f stable=%.4f mean(φm1)=%.4f", kind,
