...
ENV["GABLS1_SLD_STABILITY_STRENGTH"] = "4.0"
...
run_directory = joinpath(gate_root, "surface_layer_stab4p0")
setup = build_simulation(; run_directory)
expected_id = "gabls1_n032_weno9_surface_layer_t300_s1_rf1p0_native_stab4p0_wallf300"
stop_seconds = parse(Float64, ENV["GABLS1_SLD_STOP_SECONDS"])

@testset "λ=4 stability-corrected SLD GABLS1 gate" begin
    model = setup.model
    ...
    @test setup.settings.stability_strength == 4
    @test closure.resolved_transport isa Val{:scheme_native}
    @test closure.stability_strength == 4
    ...
    @test φᵐ ≈ 1 .+ 4f0 * 4.8f0 .* ζ rtol=1f-5
    @test φʰ ≈ 1 .+ 4f0 * 7.8f0 .* ζ rtol=1f-5
    ...
    jldopen(series) do file
        @test file["metadata/surface_layer_stability_strength"] == 4
