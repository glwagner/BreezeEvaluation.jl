...
using TOML, SHA
include(joinpath(@__DIR__, "..", "gpu_validation", "admit_resolved_flux_factor.jl"))
...
function finalize(root, evidence, job, runs, logs, destination)
    ...
    admit_resolved_flux_factor(evidence, root)
    evaluation = joinpath(root, "source", "BreezeEvaluation.jl")
    registry_relative = "cases/surface_layer/registries/gabls1_sld_resolved_factor.toml"
    scientific_path = joinpath(evaluation, registry_relative)
    scientific = TOML.parsefile(scientific_path)
    wrapper = joinpath(evaluation, "cases/surface_layer/registries/run_resolved_factor_array.slurm")
    ...
    registry = Dict{String, Any}(
        ...
        "expected_case_count" => 2, "scientific_registry_relative" => registry_relative,
    ...
    for (index, case) in enumerate(scientific["cases"])
        ...
        log = joinpath(logs, "gabls1_factor_$(job)_$(index).out")
        exit_path = joinpath(logs, "gabls1_factor_$(job)_$(index).exit")
        exit_record = parse_fields(exit_path)
        length(readlines(exit_path)) == 10 || error("wrong exit record size")
        exit_record["array_job_id"] == job && exit_record["task_id"] == string(index) || error("wrong exit identity")
        ...
        occursin("SLD_FACTOR_BATCH_EXIT job=$job task=$index child_exit_code=0 record=$exit_path", text) || error("missing durable-exit log")
        ...
        push!(attempts, Dict{String, Any}(
            ...
            "run_directory" => directory, "log_path" => log, "job_spec" => "$(job)_$index",
