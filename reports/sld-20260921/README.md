...
[Read the matched analysis](surface_layer/gabls1/native_flux/results.md) or its [illustrated PDF](surface_layer/gabls1/native_flux/results_section.pdf). A completed 12.5 m WENO9 run uses the exact operator flux in the one-face/300 s factor-one closure estimate, paired with the admitted covariance factor-one case. The 8–9 h mean first-face numerical correction is 2.95% of the u-momentum covariance and negligible for heat. Coefficients and resolved turbulence change modestly; the new approach does not resolve the gap to the fixed 1 m LES median in this single realization. Julia audit/export, compact histories, and three Julia figures accompany the result. The master report is now 85 pages and retains all previous chapters.
