# Post-run scientific-semantics audit of a NON-SCIENTIFIC 120 s neutral GPU fixture.
# This lives outside the read-only a14c358 source snapshot.
...
scalar(file, name, key) = only(vec(Float64.(file["timeseries/$name/$key"])))
...
function audit_neutral_speed_gate(output_directory)
    check_sources()
    ...
    require_neutral(!ispath(joinpath(output_directory, "NEUTRAL_SPEED_GATE_DONE")),
                    "refusing to overwrite a completed speed gate")
    ...
    report = Dict{String, Any}(
        "schema_version" => 1,
        "mode" => "gpu_neutral_120s_speed_and_flux_semantics",
        "all_passed" => true,
        ...
        "cases" => cases)
    report_path = joinpath(output_directory, "neutral_speed_gate_evidence.toml")
    open(report_path, "w") do io
        TOML.print(io, report; sorted=true)
    end
    open(joinpath(output_directory, "NEUTRAL_SPEED_GATE_DONE"), "w") do io
        println(io, "mode=gpu_neutral_120s_speed_and_flux_semantics")
        println(io, "fixture_non_scientific=true")
        println(io, "evidence_sha256=", file_sha(report_path))
    end
    println("NEUTRAL_SPEED_GATE_PASSED evidence_sha256=", file_sha(report_path))
end

abspath(PROGRAM_FILE) == abspath(@__FILE__) &&
    audit_neutral_speed_gate(only(ARGS))
