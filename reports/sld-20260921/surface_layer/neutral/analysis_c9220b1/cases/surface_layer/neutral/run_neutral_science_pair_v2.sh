#!/bin/bash
# Versioned portable neutral pair launcher. Root reviews/submits; pane 48 does not.
set -euo pipefail

case_done_valid() {
    local path="$1" expected_case="$2" line
    local case_seen=0 time_seen=0
    [[ -f "$path" ]] || return 1
    while IFS= read -r line || [[ -n "$line" ]]; do
        [[ "$line" == "case_id=$expected_case" ]] && case_seen=1
        [[ "$line" =~ ^final_time_s=18000(\.0+)?$ ]] && time_seen=1
    done < "$path"
    [[ "$case_seen" == 1 && "$time_seen" == 1 ]]
}

if [[ "${1:-}" == "--check-case-done" ]]; then
    [[ "$#" == 3 ]] || exit 2
    case_done_valid "$2" "$3"
    exit $?
fi
...
if [[ "$child_exit_code" == 0 ]]; then
    if ! case_done_valid "${run_dir}/CASE_DONE" "$case_id"; then
        child_exit_code=3
    fi
fi
if [[ "$child_exit_code" != 0 ]]; then
