...
const three=S.SurfaceLayerAnalysisData.load_case_export(joinpath(D,"exports_v1/gabls1_n032_weno9_surface_layer_t300_s1_rf3p0"))
const ten=S.SurfaceLayerAnalysisData.load_case_export(joinpath(D,"../resolved_factor10/exports_v2/gabls1_n032_weno9_surface_layer_t300_s1_rf10p0"))
...
const cases=[pair[1],three]
const colors=["#0072B2","#D55E00"]
const styles=[:dash,:solid]
const labels=["Factor 1","Factor 3"]
const markers=[:circle,:utriangle]
...
function drawprofile!(ax,var,refvar;maximum_height=250.,skipwall=false)
 for (i,c) in enumerate(cases)
  ...
  lines!(ax,p.value[ix],p.z_m[ix];color=colors[i],linestyle=styles[i],linewidth=i==2 ? 4.2 : 2.5,label=labels[i])
...
Label(f[0,1:3],"Factor 3: mean profiles and resolved turbulence",fontsize=28,font=:bold)
...
Legend(f[4,1:3],firstax;orientation=:horizontal,nbanks=1,labelsize=18)
Label(f[5,1:3],"Factor 1 is blue, dashed, circles; factor 3 is vermilion, solid, triangles.\nNative heights and physical moments retained. Fixed 1 m reference medians shown where available.",fontsize=17,tellwidth=false)
...
Label(f[1,1:2],"One supported face at 12.5 m · factors 1 and 3 · factor 1 switch-off fractions unavailable",fontsize=18)
for (col,(variable,title)) in enumerate((("surface_layer_face1_viscosity","Momentum viscosity (m² s⁻¹)"),("surface_layer_face1_ρθ_diffusivity","Heat diffusivity (m² s⁻¹)")))
 ...
 for (i,c) in enumerate(cases)
  lines!(ax,c.series.time_s./3600,c.series.values[variable];color=colors[i],linestyle=styles[i],linewidth=i==2 ? 3.5 : 2,label=labels[i])
 end
 vspan!(ax,7,8;color=(:black,0.045));vspan!(ax,8,9;color=(colors[2],0.05));xlims!(ax,0,9)
 col==1 && (global coefficient_axis=ax)
end
fractionstyles=[:solid,:dash,:dot]
fractioncolors=["#0072B2","#D55E00","#009E73"]
fractionlabels=["deficit = 0","valid AND deficit = 0","coefficient = 0"]
for (col,(prefix,coefficient,title)) in enumerate((("momentum","viscosity","Momentum: factor 3 switch-off"),("ρθ","ρθ_diffusivity","Heat: factor 3 switch-off")))
 ...
 for i in 1:3
  lines!(ax,three.series.time_s./3600,three.series.values[names[i]];color=fractioncolors[i],linestyle=fractionstyles[i],linewidth=2.5,label=fractionlabels[i])
 end
