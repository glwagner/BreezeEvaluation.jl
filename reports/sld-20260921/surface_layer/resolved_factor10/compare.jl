...
@assert hashfile(joinpath(D,"evidence_v2/gpu/validation_evidence.toml")) == GPU_SHA
const gpu_evidence = TOML.parsefile(joinpath(D,"evidence_v2/gpu/validation_evidence.toml"))
...
@assert collection["attempt_registry_sha256"] == "d92a62ac01b2fa45165c4a6c1c086758e6dc5289c846437e4ab9a92542a4c12b"
...
for factor in (1, 2, 10)
    ...
    this_collection = factor == 10 ? collection : old_collection
    entry = only(filter(e -> e["case_id"] == id, this_collection["admitted"]))
    directory = joinpath(D, factor == 10 ? "exports_v2" : "../resolved_factor/exports_v2", id)
    ...
    @assert p["source_freeze_manifest_sha256"] == p["analysis_freeze"]["manifest_sha256"] == (factor == 10 ? SOURCE_SHA : OLD_SOURCE_SHA)
    @assert p["scientific_registry_sha256"] == (factor == 10 ? "d1f4e56ed705ce035207d9900e8da441cb8f4d3a06a283acaa4958e08f314519" : OLD_REGISTRY_SHA)
    @assert p["gpu_validation"]["evidence_sha256"] == (factor == 10 ? GPU_SHA : OLD_GPU_SHA)
...
for attempt in attempts["attempts"]
    ...
    exitpath = joinpath(D, "logs_v2", basename(attempt["batch_exit_record_path"]))
...
open(joinpath(D, "comparison_profiles.csv"), "w") do io
    ...
    for label in ("factor1", "factor2", "factor10", "historical_oneface300", "historical_control")
        c = cases[label]
...
const ten_series=cases["factor10"].series
...
for hour in 1:9
    ...
    hourly[string(hour)]=Dict(name=>mean(ten_series.values[name][ix]) for name in names)
end
summary = Dict("cases"=>metrics, "differences"=>differences,
    "factor10_switch_off_series"=>fractions,
    "factor10_hourly_switch_off"=>hourly,
    "historical_factor1_reproducibility"=>equivalence, "audit"=>audit,
    "source_manifest_sha256"=>SOURCE_SHA, "registry_sha256"=>cases["factor10"].manifest["provenance"]["scientific_registry_sha256"],
    ...
    "definitions"=>Dict("profiles"=>"Equal average of the two true 1800s averages per hour; native model heights retained",
      ...
      "switch_off_fractions"=>"Factor10 only. Six saved horizontal fractions distinguish raw zero deficit, guard-valid zero deficit and actual zero coefficient for momentum/heat; denominator is every horizontal point. Factors1/2 unavailable, not zero.",
      "historical"=>"7293 data from earlier source, same grid/seed/setup; equivalence tested explicitly for oneface300",
      "scheduler"=>"Factor10 job7339 used a non-login wrapper and its durable child record verifies exit0. Older factor2 job7331 had batchNonZeroExitCode despite child0 and verified complete output; separate logout probe reproduced that older discrepancy, actual batchSHLVL unavailable."))
open(io -> JSON.print(io, summary, 2), joinpath(D, "comparison.json"), "w")
open(joinpath(D, "comparison.md"), "w") do io
    println(io, "Factors1/2 passed their original strict admission (2 admitted,0 rejected); factor10 passed separate source-specific admission (1 admitted,0 rejected). Same Breeze physics, grid and initial profiles; diagnostic-only evaluation revision for factor10. Exact schedules, physical flux partition and support rechecked locally.")
    ...
    println(io, "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|")
    for (window, _, _, _) in WINDOWS, label in ("factor1", "factor2", "factor10", "historical_oneface300", "historical_control")
    ...
    for name in fractions
        @printf(io,"| %s | %.7f | %.7f |\n",name,metrics["factor10"]["penultimate_hour"][name],metrics["factor10"]["final_hour"][name])
    end
...
println(JSON.json(Dict("final_factor1"=>metrics["factor1"]["final_hour"], "final_factor2"=>metrics["factor2"]["final_hour"], "final_factor10"=>metrics["factor10"]["final_hour"], "equivalence"=>equivalence)))
