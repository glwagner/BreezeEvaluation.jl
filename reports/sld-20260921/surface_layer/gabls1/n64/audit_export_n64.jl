...
# Adapted for 64³ GABLS1 with a paired 32³ initial perturbation field and optional γ=2 SLD.
...
for required in ("case_id: $case_id", "nx: 64", "spacing: 6.25", "filter_seconds: 300.0",
                 "wall_filter_seconds: 300.0", "stop_time: 32400.0", "seed: 123",
                 "theta_initial_sha256: 1b6c480e8e682c97ade3db99bcf9559cf1723d500858c600b74c318f4a97c864")
        ...
        for name in names(f)
            ...
            if kind == "statistics"
                ...
                for (key, t) in rs
                    ...
                    length(raw) in (64, 65) || error("$name native vertical levels")
...
jldopen(wall_path, "r") do f
    ...
    for name in ("filtered_u", "filtered_v", "filtered_Δθ")
        ...
        for (key, t) in rs
            ...
            size(a)[1:2] == (74, 74) || error("$name wall halo shape")
...
for (window, source_times) in (("final_hour", (30600.0, 32400.0)),
    ...
    open(joinpath(out_dir, "profiles_$(window)_long.csv"), "w") do io
        ...
        for name in sort!(collect(keys(profile_data)))
            ...
            location = length(a) == 64 ? "Center" : "Face"
            for i in eachindex(a)
                z = location == "Center" ? (i - 0.5) * 6.25 : (i - 1) * 6.25
