# Compare λ=4 with the admitted λ=0 and λ=1 filtered-SLD cases, filtered no closure,
# and the fixed 1 m LES median. Inputs are audited exports; nothing is rerun.
#
#   julia --project=julia compare_gamma4.jl OUTPUT_DIRECTORY
#
# Paths default to the compact data persisted in this report and may be overridden by environment.
using CairoMakie, JSON, Statistics, Printf

const D = @__DIR__
const FILTERED = joinpath(D, "..", "filtered_wall", "export")
const STABILITY = joinpath(D, "..", "stability")
const GAMMA4 = D
const REFERENCE_JSON = get(ENV, "REFERENCE_JSON",
    joinpath(D, "..", "..", "..", "gabls", "reference_data", "fixed_1m_medians.json"))

length(ARGS) == 1 || error("usage: compare_stability.jl OUTPUT_DIRECTORY")
...
const CASES = (
    ...
    ("Filtered · SLD λ=1", get(ENV, "LAMBDA1_EXPORT", joinpath(STABILITY, "export")), "#E69F00"),
    ("Filtered · SLD λ=4", get(ENV, "GAMMA4_EXPORT", joinpath(GAMMA4, "export")), "#0072B2"))
const SLD_CASES = CASES[2:4]
...
Label(f[1, 1:3], "32³ · 12.5 m · WENO9 · filtered wall · scheme-native one-face SLD · λ=0, 1, 4 · dashed 7–8 h, solid 8–9 h", fontsize=19)
...
# First-face SLD coefficients and the local stability state of the λ=4 run.
...
lines!(φ_axis, st("time_s") ./ 3600, st("mean_phi_m"); color="#0072B2", linewidth=2.6, label="φₘ")
lines!(φ_axis, st("time_s") ./ 3600, st("mean_phi_h"); color="#0072B2", linestyle=:dash, linewidth=2.6, label="φₕ")
axislegend(φ_axis; position=:rb); xlims!(φ_axis, 0, 9)
fraction_axis = Axis(f[3, 2]; xlabel="Time (h)", ylabel="Column fraction")
lines!(fraction_axis, st("time_s") ./ 3600, st("stable_fraction"); color="#0072B2", linewidth=2.6, label="stable (corrected)")
...
for (row, (stem, unit)) in enumerate((("u", "m² s⁻²"), ("ρθ", "K m s⁻¹")))
    ...
    ax2 = Axis(f[1+row, 2]; xlabel="Flux at 12.5 m ($unit)", ylabel="",
               ...
               yticks=(1:9, vcat([[n * " resolved", n * " SGS", n * " total"] for n in ("λ=0", "λ=1", "λ=4")]...)))
    ...
    barplot!(ax2, 1:9, values; direction=:x, color=colors)
end
