...
for (name, digest) in old
    (name in changed || startswith(name, "surface_layer/gabls1/n128/")) && continue
...
new = Dict{String, String}(name => digest for (name, digest) in old
                          if !startswith(name, "surface_layer/gabls1/n128/"))
for name in changed
    new[name] = sha(joinpath(root, name))
end
base = joinpath(root, "surface_layer/gabls1/n128")
