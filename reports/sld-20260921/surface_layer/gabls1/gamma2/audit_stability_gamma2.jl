# Independent audit of the saved local SLD stability state of a completed λ=2 GABLS1 run.
...
const λ, βᵐ, βʰ, z₁ = 2.0, 4.8, 7.8, 12.5
const minimum_stress, heat_guard = 1e-8, 1e-8  # (1e-4 m/s)², ρθ guard of the case
interior_xy(a) = size(a, 1) == 32 ? a[:, :, 1] : a[6:37, 6:37, 1]
...
summary = Dict{String, Any}("case_id" => case_id, "records" => length(rows),
    "stored_inverse_length_matches_saved_fluxes" => true, "phi_identities_pass" => true)
