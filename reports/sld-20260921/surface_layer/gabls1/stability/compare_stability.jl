# Compare the λ=1 stability-corrected SLD GABLS1 case with the admitted λ=0 filtered-SLD
# control, the filtered no-closure case, and the fixed 1 m LES median. Inputs are the audited
# exports written by audit_export.jl (and audit_stability.jl for λ=1); nothing is rerun.
#
#   julia --project=julia compare_stability.jl OUTPUT_DIRECTORY
#
...
Label(f[1, 1:3], "32³ · 12.5 m · WENO9 · filtered wall · scheme-native one-face SLD · λ=0 vs λ=1 · dashed 7–8 h, solid 8–9 h", fontsize=19)
...
# First-face SLD coefficients and the local stability state of the λ=1 run.
stability_rows = rows(joinpath(CASES[3][2], "stability_series.csv"))
...
lines!(φ_axis, st("time_s") ./ 3600, st("mean_phi_m"); color="#E69F00", linewidth=2.6, label="φₘ")
lines!(φ_axis, st("time_s") ./ 3600, st("mean_phi_h"); color="#E69F00", linestyle=:dash, linewidth=2.6, label="φₕ")
axislegend(φ_axis; position=:rb); xlims!(φ_axis, 0, 9)
fraction_axis = Axis(f[3, 2]; xlabel="Time (h)", ylabel="Column fraction")
lines!(fraction_axis, st("time_s") ./ 3600, st("stable_fraction"); color="#E69F00", linewidth=2.6, label="stable (corrected)")
...
for (row, (stem, unit)) in enumerate((("u", "m² s⁻²"), ("ρθ", "K m s⁻¹")))
    ...
    ax2 = Axis(f[1+row, 2]; xlabel="Flux at 12.5 m ($unit)", ylabel="",
               ...
               yticks=(1:6, vcat([[n * " resolved", n * " SGS", n * " total"] for n in ("λ=0", "λ=1")]...)))
    ...
    barplot!(ax2, 1:6, values; direction=:x, color=colors)
end
...
open(joinpath(OUT, "metrics.md"), "w") do io
    ...
    println(io, "\n## λ=1 local stability state (horizontal means of local values)\n")
