...
paragraphs=String[
    ...
    "The final-hour mean horizontal-wind vector shear across 6.25-18.75 m is 0.0522 s^-1 for scheme-native SLD and 0.0511 s^-1 for covariance SLD, versus 0.1004 s^-1 from the fixed 1 m median profiles; the no-closure case is high at 0.1861 s^-1. The pattern persists during 7-8 h, but differences change sign higher up. All model gradients use adjacent 12.5 m cell-center levels. Reference componentwise median u/v profiles are sampled at those heights and differentiated identically; this is not the median of member shears or a wall gradient.",
...
append!(pages,[joinpath(D,"figures",n*".pdf") for n in ("native_profiles","native_activity","native_partition","gabls1_shear")])
