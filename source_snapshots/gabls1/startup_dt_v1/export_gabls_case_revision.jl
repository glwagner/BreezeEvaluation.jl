...
length(ARGS) == 11 || error(
    "usage: export_gabls_case_revision.jl PRODUCTION_ROOT CASE_ID LOG_PATH " *
    "SLURM_JOB_SPEC RUN_DIR SOURCE_DIR SOURCE_REVISION MANIFEST_SHA " *
    "DIAGNOSTICS_SHA RUNNER_SHA SURFACE_SHA")

const PRODUCTION_ROOT = normpath(ARGS[1])
const CASE_ID = ARGS[2]
const LOG_PATH = abspath(ARGS[3])
const SLURM_JOB_SPEC = ARGS[4]
const RUN_DIR = normpath(ARGS[5])
const SOURCE_DIR = normpath(ARGS[6])
const SOURCE_REVISION = ARGS[7]
const EXPECTED_SOURCE_HASHES = Dict(
    "manifest" => ARGS[8], "diagnostics" => ARGS[9],
    "runner" => ARGS[10], "surface" => ARGS[11])
...
function verify_source_hashes()
    pairs = [
        ("diagnostics", joinpath(SOURCE_DIR, "examples/gabls_diagnostics.jl"),
                        joinpath(RUN_DIR, "provenance/gabls_diagnostics.jl")),
        ("runner", joinpath(SOURCE_DIR, "validation_output/gabls/gabls_case.jl"),
                   joinpath(RUN_DIR, "provenance/gabls_case.jl")),
        ("surface", joinpath(SOURCE_DIR, "src/BoundaryConditions/gabls_rough_wall_coefficient.jl"),
                    joinpath(RUN_DIR, "provenance/gabls_rough_wall_coefficient.jl")),
        ("manifest", joinpath(SOURCE_DIR, "Manifest.toml"),
    ...
    for (name, frozen, captured) in pairs
        ...
        frozen_hash == expected_hash || error(
            "Registry source hash mismatch for $name: expected $expected_hash, source has $frozen_hash")
...
function export_case(destination)
    ...
    manifest = Dict{String, Any}(
        ...
        "source_directory" => relpath(SOURCE_DIR, PRODUCTION_ROOT),
        "export_verified" => true,
        "source_hashes_checked" => true,
        "registry_source_hashes_checked" => true,
